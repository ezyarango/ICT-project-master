...
#game over screen
def game_over():
    global gameover, high_score, player_score, player_x, player_y, obstacles, running, last_x, obstacle_speed
    while not running:
        SCREEN= pygame.image.load("nightforest.jpg")
        screen.blit(SCREEN,(0,0))
        display_game_over = font.render ("GAME OVER!", True, yellow )
        screen.blit(display_game_over, (80, 100))
        text= font2.render("Press the Space key to restart", True, yellow)
        # back= font2.render("Back to Main Menu", True, yellow)
        points= font2.render("Your score: " + str(player_score), True, yellow)
        screen.blit(points, (350,85))
        textRect= text.get_rect()
        textRect.center = (screen_width // 2, screen_height // 1.8)
        # menuRect= back.get_rect()
        # menuRect.center = (screen_width // 2, screen_height // 1.5)
        # screen.blit(back, menuRect)
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_SPACE and gameover:
                    ...
                    obstacles = []
                    last_x = 0  # Reset obstacle spawn position
                    obstacle_speed = 5  # Reset speed (optional)
...
def update_score():
    global player_score, game_speed, yellow, font2
    player_score+=1

    if player_score % 1000 == 0:
        game_speed +=1
   
    currentscore = font2.render("Score: " + str(player_score), True, yellow)
    textRect = currentscore.get_rect()
    textRect.center = (828, 55)
    screen.blit(currentscore, textRect)
    highestscore = font2.render("High score: " + str(high_score), True, yellow)
    textRect = highestscore.get_rect()
    textRect.center = (805, 25)
    screen.blit(highestscore, textRect)
...
def check_difficulty():
    ...
    x_mod = (player_score//50)
    y_mod = (player_score//20)
    if obstacle_speed > 0:
        obstacle_speed = 4 + x_mod
